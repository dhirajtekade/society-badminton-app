...
export default function AdminLeaderboardPage() {
    ...
    const fetchLeaderboardData = async () => {
      ...
      try {
        ...
        matchesSnap.forEach((doc) => {
          ...
          // --- SMART POINT ACCUMULATION FOR MULTI-SET OR SINGLE-SET MATCHES ---
          if (m.sets && Array.isArray(m.sets) && m.sets.length > 0) {
            m.sets.forEach((set) => {
              matchPointsA += set.scoreA || 0;
              matchPointsB += set.scoreB || 0;
            });
          } else {
            matchPointsA = m.scoreA || 0;
            matchPointsB = m.scoreB || 0;
          }
          ...
          if (walkover === "A") teamAWon = true;
            ...
            // For multi-set matches, scoreA/scoreB stores sets won (e.g. 2 vs 1)
            teamAWon = (m.scoreA || 0) > (m.scoreB || 0);
            teamBWon = (m.scoreB || 0) > (m.scoreA || 0);
          }

          teamA.forEach((id) => {
            if (stats[id]) {
              stats[id].matchesPlayed += 1;
              stats[id].pointsScored += walkover ? 0 : matchPointsA;
              stats[id].pointsConceded += walkover ? 0 : matchPointsB;
          ...
          teamB.forEach((id) => {
            if (stats[id]) {
              stats[id].matchesPlayed += 1;
              stats[id].pointsScored += walkover ? 0 : matchPointsB;
              stats[id].pointsConceded += walkover ? 0 : matchPointsA;
        ...
        Object.values(stats).forEach((p) => {
          ...
          p.rankScore =
            p.totalWins * 10000 + p.pointDifference * 100 + p.pointsScored;
        });

        const overallStandings = Object.values(stats).sort(
  ...
  const renderTable = (title, players) => (
    <div
      key={title}
      className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500"
    >
        ...
        <table className="w-full text-left border-collapse text-sm whitespace-nowrap">
          <thead>
            <tr className="bg-gray-800 text-white text-xs uppercase tracking-wider">
              <th className="p-3.5 border-r border-gray-700 w-16 text-center">
                Rank
              </th>
              <th className="p-3.5 border-r border-gray-700">Player Name</th>
              {viewMode === "overall" && (
                <th className="p-3.5 border-r border-gray-700">Tag</th>
              )}
              <th className="p-3.5 border-r border-gray-700 text-center">
                Matches
              </th>
              <th className="p-3.5 border-r border-gray-700 text-center">
                Total Wins
          ...
          <tbody className="divide-y divide-gray-200 font-medium">
            {players.map((p, index) => (
              <tr key={p.id} className="hover:bg-gray-50">
                ...
                <td className="p-3.5 border-r border-gray-200 font-bold text-gray-900">
                  {p.name}
                </td>
                {viewMode === "overall" && (
                ...
                <td className="p-3.5 border-r border-gray-200 text-center text-gray-600">
                  {p.matchesPlayed}
                </td>
                <td className="p-3.5 border-r border-gray-200 text-center font-bold text-green-600">
                  {p.totalWins}
                </td>
                <td className="p-3.5 border-r border-gray-200 text-center text-gray-700">
                  {p.pointsScored}
                </td>
                <td className="p-3.5 border-r border-gray-200 text-center text-gray-700">
                  {p.pointsConceded}
                ...
                <td
                  className={`p-3.5 border-r border-gray-200 text-center font-bold font-mono ${p.pointDifference > 0 ? "text-green-600" : p.pointDifference < 0 ? "text-red-600" : "text-gray-600"}`}
                >
                  {p.pointDifference > 0
                    ? `+${p.pointDifference}`
                    : p.pointDifference}
                </td>
                <td className="p-3.5 text-center font-mono font-bold text-indigo-700 bg-indigo-50/50">
                  {p.rankScore}
                </td>
              </tr>
            ))}
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-6 mt-6">
      ...
      {isLoading ? (
        <div className="p-12 text-center text-gray-500 font-medium">
          Calculating standings from match ledger...
        </div>
        ...
        <div className="space-y-2">
          {viewMode === "overall"
            ...
            : Object.entries(standings.divisions).map(
