...
import { Plus, Edit2, Clock, Zap, AlertTriangle, Settings2, X, Trash2, ArrowUp, ArrowDown, Trophy } from "lucide-react";
import { useTournament } from "@/components/TournamentSelector";

export default function AdminMatchesPage() {
  const { tournaments, activeTournament, switchTournament, createTournament, isLoading: tLoading } = useTournament();
  ...
  const [showCategory, setShowCategory] = useState(false); // Toggle for player categories
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // 1. PULL MASTER DATABASE FIRST (The Single Source of Truth)
        ...
        // 2. PULL TOURNAMENT ROSTER & FORCE MASTER OVERRIDES
        const playersSnap = await getDocs(collection(db, "tournaments", activeTournament.id, "players"));
        const playersDict = {};
        const pList = [];
        
        playersSnap.forEach((doc) => {
          const tData = doc.data();               // Tournament enrollment data
          const mData = masterDict[doc.id] || {}; // Master global data
          
          // Strict Merge: Tournament tells us IF they play, Master tells us WHO they are.
          const mergedData = { 
            id: doc.id,
            playsSingles: tData.playsSingles || false,
            playsDoubles: tData.playsDoubles || false,
            name: mData.name || tData.name || "Unknown", 
            category: mData.category || "Uncategorized",  // <--- STRICT MASTER OVERRIDE
            availability: mData.availability || []        // <--- STRICT MASTER OVERRIDE
          };
  ...
  // HELPER: Trims the ending time off the slot
  ...
  const handleClearAllMatches = async () => {
    ...
    try {
      ...
      matchesSnap.forEach((matchDoc) => batch.delete(doc(db, "tournaments", activeTournament.id, "matches", matchDoc.id)));
      await batch.commit();
      setMatches([]);
      alert("All tournament fixtures deleted successfully.");
    } catch (error) {
  ...
  const executeSmartGeneration = async (e) => {
    ...
    try {
      ...
      const totalSlotTime = (parseInt(matchDuration, 10) || 10) + (parseInt(bufferTime, 10) || 5);
      ...
      if (rules.sameCategory) {
        const uniqueCategories = [...new Set(activePlayers.map(p => p.category?.trim() || "Uncategorized"))];
        
        uniqueCategories.forEach(catName => {
          const catPlayers = activePlayers.filter(p => (p.category?.trim() || "Uncategorized") === catName);
          
          for (let i = 0; i < catPlayers.length; i++) {
            for (let offset = 1; offset < catPlayers.length; offset++) {
              const j = (i + offset) % catPlayers.length;
              if (i !== j) tryAddPair(catPlayers[i], catPlayers[j], `League: ${catName}`);
            }
          }
        });
      } else {
        for (let i = 0; i < activePlayers.length; i++) {
          for (let offset = 1; offset < activePlayers.length; offset++) {
            ...
            if (i !== j) tryAddPair(activePlayers[i], activePlayers[j], 'General League');
          }
        }
      }
      ...
      tournamentDays.forEach(day => {
        day.slots.forEach(block => {
          ...
          while (currentTotalMins + (parseInt(matchDuration, 10) || 10) <= endTotalMins) {
            const mStartH = Math.floor(currentTotalMins / 60);
            const mStartM = currentTotalMins % 60;
            
            const matchEndMins = currentTotalMins + (parseInt(matchDuration, 10) || 10);
      ...
      pairs.forEach(pair => {
        ...
        for (const key of slotKeys) {
          ...
          const p1Avail = (pair.p1.availability && Array.isArray(pair.p1.availability) && pair.p1.availability.length > 0) 
            ? pair.p1.availability 
            : allParentBlockIds;

          const p2Avail = (pair.p2.availability && Array.isArray(pair.p2.availability) && pair.p2.availability.length > 0) 
            ? pair.p2.availability 
            : allParentBlockIds;

          const hasAvailability = !rules.checkAvail || (p1Avail.includes(slot.parentBlockId) && p2Avail.includes(slot.parentBlockId));
        ...
        const docRef = doc(collection(db, "tournaments", activeTournament.id, "matches"));
      ...
      let summaryText = `Success! Generated ${generatedMatches.length} total matches.\n\n`;
      
      if (rules.sameCategory) {
        const uniqueCategories = [...new Set(activePlayers.map(p => p.category?.trim() || "Uncategorized"))];
        uniqueCategories.forEach(cat => {
          const pCount = activePlayers.filter(p => (p.category?.trim() || "Uncategorized") === cat).length;
          const mCount = generatedMatches.filter(m => m.stage === `League: ${cat}`).length;
          if (pCount > 0) {
            summaryText += `• ${cat}: ${pCount} Players ➔ ${mCount} Matches\n`;
          }
        });
      } else {
        summaryText += `• General League: ${activePlayers.length} Players ➔ ${generatedMatches.length} Matches\n`;
      }
  ...
  const handleSaveMatch = async (e) => {
    ...
    try {
      if (editingMatchId) {
        await setDoc(doc(db, "tournaments", activeTournament.id, "matches", editingMatchId), matchData, { merge: true });
        setMatches(prev => prev.map(m => m.id === editingMatchId ? { id: editingMatchId, ...matchData } : m));
      } else {
        const docRef = await addDoc(collection(db, "tournaments", activeTournament.id, "matches"), matchData);
  ...
  const filteredAndSortedMatches = [...matches].filter(match => {
    const matchesDate = filterDate === "all" || (match.timeSlot && match.timeSlot.includes(formatDisplayDate(filterDate)));
    ...
    if (sortBy === "time") {
      const parseTimeToMinutes = (timeSlotStr) => {
        ...
        try {
          const timePart = timeSlotStr.split('|')[1]?.trim().split('-')[0]?.trim(); 
      ...
      comparison = parseTimeToMinutes(a.timeSlot) - parseTimeToMinutes(b.timeSlot);
  ...
  const copyWhatsAppSchedule = () => {
    ...
    Object.entries(groupedMatches).forEach(([groupTitle, groupMatches]) => {
      ...
      groupMatches.forEach((m, idx) => {
        // Uses getPlayerDisplay to include categories if the toggle is checked
      ...
      <div className="max-w-7xl mx-auto p-6 mt-10">
        ...
        <div className="bg-indigo-900 text-white p-5 rounded-xl shadow-md mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            <div className="bg-white/10 p-3 rounded-lg"><Trophy size={28} className="text-yellow-400" /></div>
            <div>
              <div className="text-xs font-bold uppercase tracking-wider text-indigo-300">Active Tournament Context</div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">Match Schedule & Editor</h2>
            <p className="text-gray-500 mt-1">Managing fixtures for {activeTournament?.name}.</p>
          </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-6 flex flex-wrap gap-4 items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {tournamentDays.map(day => (
                  <option key={day.date} value={day.date}>{formatDisplayDate(day.date)}</option>
                ))}
              </select>
            </div>
          ...
          <div className="text-sm font-semibold text-gray-500">
            Showing <span className="text-indigo-600 font-bold">{filteredAndSortedMatches.length}</span> matches
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6">
          {isLoading ? (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center text-gray-500">Loading matches...</div>
          ) : filteredAndSortedMatches.length === 0 ? (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center text-gray-500">No matches found for this tournament.</div>
          ) : (
            Object.entries(groupedMatches).map(([groupTitle, matchesInGroup]) => (
              <div key={groupTitle} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                
                {/* --- GROUP HEADER --- */}
                <div className="bg-indigo-50/80 p-4 border-b border-indigo-100 flex items-center justify-between">
                  <h3 className="font-black text-indigo-900 uppercase tracking-wider">{groupTitle}</h3>
                  <span className="text-xs font-bold text-indigo-600 bg-indigo-100 px-2 py-1 rounded">{matchesInGroup.length} Matches</span>
                </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-100 text-sm">
                      {matchesInGroup.map((match) => (
                        <tr key={match.id} className={`hover:bg-gray-50/80 transition-colors ${match.status === 'conflict' ? 'bg-red-50/40' : ''}`}>
                          <td className="p-4 font-semibold text-gray-800">
                            {match.teamA.map((id, idx) => <div key={idx}>{getPlayerDisplay(id)}</div>)}
                          </td>
                          <td className="p-4 text-center font-bold text-gray-300">VS</td>
                          <td className="p-4 font-semibold text-gray-800">
                            {match.teamB.map((id, idx) => <div key={idx}>{getPlayerDisplay(id)}</div>)}
                          </td>
                          <td className="p-4 text-gray-600 font-medium whitespace-nowrap">
                            <div className="flex items-center gap-1.5">
                              {match.status === 'conflict' && <AlertTriangle size={14} className="text-red-500" />}
                              <Clock size={14} className="text-gray-400" /> 
                              {formatStartTime(match.timeSlot)}
                            </div>
                          </td>
                          <td className="p-4 text-right">
                            <button onClick={() => openEditModal(match)} className="text-gray-400 hover:text-blue-600 p-1.5 rounded-lg hover:bg-blue-50 transition-colors" title="Edit Match">
                              <Edit2 size={16} />
                            </button>
                          </td>
                        </tr>
      ...
      {showNewModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[9999]">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden p-6">
            <h3 className="text-lg font-bold text-gray-800 mb-2">Create New Tournament</h3>
            <p className="text-xs text-gray-500 mb-4">Enter a name for the new tournament category/event.</p>
            <input 
              type="text" 
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowNewModal(false)} className="flex-1 px-4 py-2 border rounded-lg text-sm font-bold">Cancel</button>
              <button onClick={() => { createTournament(newTourneyName); setNewTourneyName(""); setShowNewModal(false); }} className="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold">Create</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="p-5 border-b bg-indigo-600 text-white flex justify-between items-center">
              <div>
                ...
                <p className="text-indigo-100 text-sm">Generating for {activeTournament?.name}</p>
              </div>
            ...
            <form onSubmit={executeSmartGeneration} className="p-6">
              <div className="grid grid-cols-3 gap-3 mb-5">
                <div>
                  ...
                  <select value={genData.type} onChange={e => setGenData({...genData, type: e.target.value})} className="w-full border p-2.5 rounded bg-gray-50 outline-none text-sm">
                    <option value="singles">Singles</option>
                    <option value="doubles">Doubles</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1">Max Matches</label>
                  <input type="number" min="1" max="10" value={genData.maxMatchesPerPlayer} onChange={e => setGenData({...genData, maxMatchesPerPlayer: e.target.value})} className="w-full border p-2.5 rounded bg-gray-50 outline-none text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1">Courts</label>
                  <input type="number" min="1" max="5" value={genData.courtCount} onChange={e => setGenData({...genData, courtCount: e.target.value})} className="w-full border p-2.5 rounded bg-gray-50 outline-none text-sm" />
                </div>
              </div>

              <div className="space-y-3 mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                <h3 className="font-bold text-gray-800 text-sm border-b pb-2">Rule Hierarchy</h3>
                <div>
                  <label className="block text-xs font-bold text-indigo-600 uppercase mb-1">Priority 1</label>
                  <select value={genData.priority1} onChange={e => setGenData({...genData, priority1: e.target.value})} className="w-full border p-2 rounded outline-none bg-white text-sm">
                    {ruleOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-indigo-600 uppercase mb-1">Priority 2</label>
                  <select value={genData.priority2} onChange={e => setGenData({...genData, priority2: e.target.value})} className="w-full border p-2 rounded outline-none bg-white text-sm">
                    {ruleOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Priority 3</label>
                  <select value={genData.priority3} onChange={e => setGenData({...genData, priority3: e.target.value})} className="w-full border p-2 rounded outline-none bg-white text-sm">
                    {ruleOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                  </select>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setIsGenModalOpen(false)} className="flex-1 px-4 py-3 border text-gray-700 font-bold rounded-lg hover:bg-gray-100">Cancel</button>
                <button type="submit" disabled={isGenerating} className="flex-1 px-4 py-3 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 disabled:opacity-50 flex justify-center items-center gap-2">
                  {isGenerating ? "Executing..." : <><Zap size={18} /> Generate Setup</>}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl mt-10 mb-10">
            <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-gray-500">Edit match details for {activeTournament?.name}.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSaveMatch} className="p-6">
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <select value={formData.type} onChange={e => { const newType = e.target.value; setFormData({...formData, type: newType, teamA: newType === 'singles' ? [formData.teamA[0] || ""] : [formData.teamA[0] || "", formData.teamA[1] || ""], teamB: newType === 'singles' ? [formData.teamB[0] || ""] : [formData.teamB[0] || "", formData.teamB[1] || ""]}); }} className="w-full border p-2 rounded outline-none">
                    <option value="singles">Singles</option>
                    <option value="doubles">Doubles</option>
                  </select>
                </div>
                <div><label className="block text-sm font-medium mb-1">Stage</label><input type="text" value={formData.stage} onChange={e => setFormData({...formData, stage: e.target.value})} className="w-full border p-2 rounded outline-none" /></div>
              </div>
                ...
                <div>
                  ...
                  {formData.teamA.map((id, index) => (
                    <div key={index} className="mb-2">
                      <input type="text" value={id} onChange={(e) => { const newTeam = [...formData.teamA]; newTeam[index] = e.target.value; setFormData({...formData, teamA: newTeam}); }} className="w-full border p-2 rounded mb-1 outline-none" placeholder={`Player ${index + 1} ID`} />
                      {/* Properly map through players object property "name" */}
                      <span className="text-xs text-gray-500 block truncate">Name: {players[id]?.name || "Unknown ID"}</span>
                    </div>
                  ))}
                </div>
                <div>
                  ...
                  {formData.teamB.map((id, index) => (
                    <div key={index} className="mb-2">
                      <input type="text" value={id} onChange={(e) => { const newTeam = [...formData.teamB]; newTeam[index] = e.target.value; setFormData({...formData, teamB: newTeam}); }} className="w-full border p-2 rounded mb-1 outline-none" placeholder={`Player ${index + 1} ID`} />
                      {/* Properly map through players object property "name" */}
                      <span className="text-xs text-gray-500 block truncate">Name: {players[id]?.name || "Unknown ID"}</span>
                    </div>
                  ))}
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <select value={formData.timeSlot} onChange={e => setFormData({...formData, timeSlot: e.target.value})} className="w-full border p-2 rounded outline-none">
                ...
                <div>
                  ...
                  <select value={formData.court} onChange={e => setFormData({...formData, court: e.target.value})} className="w-full border p-2 rounded outline-none">
