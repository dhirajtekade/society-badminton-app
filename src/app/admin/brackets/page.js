...
import { Network, Users, CheckSquare, Square, ChevronRight, Filter, Shuffle, ArrowLeft, Save, Trophy, Medal, RotateCcw } from "lucide-react";
...
export default function AdminBracketsPage() {
    ...
    const fetchTournamentData = async () => {
      ...
      try {
        ...
        const rankedArray = Object.values(stats).map(p => {
          ...
          const rankScore = (p.totalWins * 10000) + (pointDifference * 100) + p.pointsScored;
        ...
        // If knockout matches already exist in Firestore, direct to Bracket Tree (Step 3)
        const knockoutMatches = fetchedMatches.filter(m => 
          ["Pre-Quarter", "Round of 16", "Quarter-Finals", "Semi-Finals", "Final"].some(s => m.stage?.includes(s))
        );
  ...
  const displayedPlayers = filterCat === "all" ? rankedPlayers : rankedPlayers.filter(p => p.category === filterCat);
  ...
  // Prepares pairings per official Olympic/Grand Slam bracket seeding order
  const prepareMatchups = () => {
    ...
    if (bracketSize <= 4) setStageName("Semi-Finals");
    else if (bracketSize <= 8) setStageName("Quarter-Finals");
    else if (bracketSize <= 16) setStageName("Pre-Quarter");
    else setStageName(`Round of ${bracketSize}`);
  ...
  const handleGenerateMatches = async () => {
    ...
    try {
      ...
      matchups.forEach((m, idx) => {
        ...
        const matchRef = doc(collection(db, "tournaments", activeTournament.id, "matches"));
        const docData = {
          stage: `${stageName} (${m.code || idx + 1})`,
          type: matchType,
          teamA: [m.p1],
          teamB: [m.p2],
          status: "scheduled",
  ...
  if (tLoading) return <div className="p-10 text-center text-gray-500">Loading Bracket Builder...</div>;

  const draftedPlayersData = rankedPlayers.filter(p => selectedIds.includes(p.id));

  // Helper function to extract winner from completed match
  ...
  // Finds matches dynamically by stage name identifier
    ...
    <div className="max-w-[1600px] mx-auto p-4 md:p-6 mt-4 pb-24">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 bg-indigo-900 text-white p-4 rounded-xl shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black tracking-tight">Knockout Stage (Round of 16)</h1>
            <p className="text-xs font-bold text-indigo-300 uppercase">
              {step === 1 ? "Step 1: Draft Roster" : step === 2 ? "Step 2: Seed Pairings" : "Tournament Bracket Tree"}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {step === 3 && (
            <button 
              ...
              className="bg-indigo-800 hover:bg-indigo-700 text-indigo-200 hover:text-white px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors"
            >
              <RotateCcw size={14} /> Re-seed
            </button>
          )}
          <select 
            value={activeTournament?.id || ""} 
            onChange={e => { switchTournament(e.target.value); setSelectedIds([]); setStep(1); }}
      ...
      {isLoading ? (
        <div className="p-12 text-center text-gray-500">Loading bracket system...</div>
      ) : step === 1 ? (
        
        /* =========================================================================
           STEP 1: SELECTION POOL DRAFT
           ========================================================================= */
          ...
          <div className="bg-gray-50 p-4 border-b border-gray-200 flex flex-col md:flex-row justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {uniqueCategories.map(cat => <option key={cat} value={cat}>{cat} Tag</option>)}
              </select>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-gray-400 uppercase mr-2">Quick Select:</span>
              <button onClick={() => selectTopN(8)} className="bg-indigo-100 text-indigo-700 hover:bg-indigo-200 px-3 py-1.5 rounded text-sm font-bold transition-colors">Top 8</button>
              <button onClick={() => selectTopN(16)} className="bg-indigo-100 text-indigo-700 hover:bg-indigo-200 px-3 py-1.5 rounded text-sm font-bold transition-colors">Top 16</button>
              <button onClick={() => setSelectedIds([])} className="bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 px-3 py-1.5 rounded text-sm font-bold ml-2 transition-colors">Clear</button>
            </div>
          </div>
              ...
              return (
                <label 
                  ...
                  className={`flex items-center justify-between p-3 mb-2 rounded-lg border-2 cursor-pointer transition-all ${
                    isSelected ? 'border-indigo-500 bg-indigo-50' : 'border-gray-100 hover:border-gray-300 hover:bg-gray-50'
                  }`}
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="font-black text-gray-900 text-lg flex items-center gap-2">
                        {index + 1}. {p.name} 
                        {isSelected && <span className="bg-indigo-600 text-white text-[10px] uppercase px-1.5 py-0.5 rounded">Drafted</span>}
                      </div>
                  ...
                  <div className="text-right">
                    <div className="text-sm font-black text-indigo-700">{p.rankScore}</div>
                    <div className="text-[10px] font-bold text-gray-400 uppercase">Rank Score</div>
                  </div>
                  <input type="checkbox" className="hidden" checked={isSelected} onChange={() => togglePlayer(p.id)} />
                </label>
              );
            })}
          </div>
        ...
        /* =========================================================================
           STEP 2: MATCHUP SEEDING EDITOR
           ========================================================================= */
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden animate-in fade-in">
          <div className="bg-indigo-50 p-5 border-b border-indigo-100 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-black text-indigo-600 uppercase tracking-wider mb-1">Stage Name</label>
              <input 
                type="text" 
                value={stageName} 
            ...
            <div>
              <label className="block text-xs font-black text-indigo-600 uppercase tracking-wider mb-1">Match Type</label>
              <select 
                value={matchType} 
          ...
          <div className="p-4 md:p-6 space-y-4">
            {matchups.map((match, idx) => (
              <div key={match.id} className="bg-gray-50 border border-gray-200 rounded-xl p-4 flex flex-col md:flex-row items-center gap-4 relative">
                ...
                <div className="flex-1 w-full mt-2 md:mt-0">
                  <select 
                    value={match.p1} 
                    onChange={e => updateMatchupPlayer(idx, 'p1', e.target.value)}
                  ...
                  >
                    ...
                    {draftedPlayersData.map(p => (
                      <option key={p.id} value={p.id}>
                        {p.name} (Seed {draftedPlayersData.findIndex(d => d.id === p.id) + 1})
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="flex flex-col items-center justify-center shrink-0">
                  ...
                  <span className="text-[10px] font-bold text-gray-400 mt-1 uppercase">VS</span>
                </div>

                <div className="flex-1 w-full">
                  <select 
                    value={match.p2} 
                    onChange={e => updateMatchupPlayer(idx, 'p2', e.target.value)}
                    className="w-full border border-gray-300 p-3 rounded-lg text-sm font-bold text-gray-800 outline-none focus:border-indigo-500 bg-white shadow-sm"
                  >
                    <option value="">-- Select Player --</option>
                    <option value="BYE">BYE (Auto-Advance)</option>
        ...
        /* =========================================================================
           STEP 3: SPREADSHEET-STYLE INTERACTIVE BRACKET BOARD
           ========================================================================= */
          ...
          <div className="min-w-[1200px] grid grid-cols-5 gap-6 items-center">
            ...
            <div className="space-y-4">
              ...
              {[1, 2, 3, 4, 5, 6, 7, 8].map(num => {
                ...
                return (
                  <div key={num} className="bg-sky-50/60 border border-sky-200 rounded-lg p-2.5 shadow-sm text-xs font-semibold">
                    <div className="text-[10px] font-black text-sky-700 uppercase mb-1 flex justify-between">
                      <span>PQ {num}</span>
                      {isComplete && <span className="text-green-600">Final</span>}
                    </div>
                    <div className={`flex justify-between items-center py-1 px-1.5 rounded ${p1Win ? 'bg-sky-200/80 font-black text-gray-900' : 'text-gray-700'}`}>
                      <span className="truncate">{match?.teamA ? getPlayerName(match.teamA[0]) : "TBD"}</span>
                      <span className="font-mono">{match?.scoreA ?? "-"}</span>
                    </div>
                    <div className={`flex justify-between items-center py-1 px-1.5 rounded mt-0.5 ${p2Win ? 'bg-sky-200/80 font-black text-gray-900' : 'text-gray-700'}`}>
            ...
            <div className="space-y-12">
              ...
              {[1, 2, 3, 4].map(num => {
                ...
                // Derive auto-qualifier from PQ matches if QF match isn't finalized
                ...
                return (
                  <div key={num} className="bg-pink-50/60 border border-pink-200 rounded-lg p-3 shadow-sm text-xs font-semibold">
                    <div className="text-[10px] font-black text-pink-700 uppercase mb-1 flex justify-between">
                      <span>QF {num}</span>
                      {isComplete && <span className="text-green-600">Final</span>}
                    </div>
                    <div className={`flex justify-between items-center py-1 px-1.5 rounded ${p1Win ? 'bg-pink-200 font-black text-gray-900' : 'text-gray-700'}`}>
                      <span className="truncate">{autoP1}</span>
                      <span className="font-mono">{match?.scoreA ?? "-"}</span>
                    </div>
                    <div className={`flex justify-between items-center py-1 px-1.5 rounded mt-0.5 ${p2Win ? 'bg-pink-200 font-black text-gray-900' : 'text-gray-700'}`}>
            ...
            <div className="space-y-36">
              ...
              {[1, 2].map(num => {
                ...
                return (
                  <div key={num} className="bg-indigo-50/60 border border-indigo-200 rounded-lg p-3 shadow-sm text-xs font-semibold">
                    <div className="text-[10px] font-black text-indigo-700 uppercase mb-1 flex justify-between">
                      <span>Semi {num}</span>
                      {isComplete && <span className="text-green-600">Final</span>}
                    </div>
                    <div className={`flex justify-between items-center py-1.5 px-2 rounded ${p1Win ? 'bg-indigo-200 font-black text-gray-900' : 'text-gray-700'}`}>
                      <span className="truncate">{autoP1}</span>
                      <span className="font-mono">{match?.scoreA ?? "-"}</span>
                    </div>
                    <div className={`flex justify-between items-center py-1.5 px-2 rounded mt-1 ${p2Win ? 'bg-indigo-200 font-black text-gray-900' : 'text-gray-700'}`}>
                      <span className="truncate">{autoP2}</span>
                      <span className="font-mono">{match?.scoreB ?? "-"}</span>
                    </div>
                  </div>
                ...
                return (
                  <div className="bg-amber-50/80 border-2 border-amber-300 rounded-xl p-4 shadow-md text-xs font-semibold">
                    <div className="text-xs font-black text-amber-800 uppercase mb-2 flex items-center justify-between">
                      <span className="flex items-center gap-1.5"><Trophy size={16} className="text-amber-600" /> FINAL</span>
                      {isComplete && <span className="bg-green-600 text-white text-[10px] px-2 py-0.5 rounded">Winner Decided</span>}
                    </div>
                    <div className={`flex justify-between items-center py-2 px-2.5 rounded ${p1Win ? 'bg-amber-200 font-black text-gray-900 text-sm' : 'text-gray-800'}`}>
                      <span className="truncate">{autoP1}</span>
                      <span className="font-mono">{match?.scoreA ?? "-"}</span>
                    </div>
                    <div className={`flex justify-between items-center py-2 px-2.5 rounded mt-1.5 ${p2Win ? 'bg-amber-200 font-black text-gray-900 text-sm' : 'text-gray-800'}`}>
                      <span className="truncate">{autoP2}</span>
                      <span className="font-mono">{match?.scoreB ?? "-"}</span>
                    </div>
                  </div>
            ...
            <div className="flex flex-col items-center justify-center p-6 bg-gradient-to-b from-yellow-50 to-amber-100/60 border-2 border-yellow-300 rounded-2xl text-center shadow-md">
              ...
              <span className="text-xs font-black text-amber-800 uppercase tracking-widest mb-1">Tournament Champion</span>
              {(() => {
                  ...
                  <div className="text-lg font-black text-gray-900 mt-1">
                    {champ ? (
                      <span className="text-indigo-950 flex items-center gap-1.5 justify-center">
                        <Medal size={20} className="text-amber-500" /> {champ.name}
                      </span>
                    ) : (
                      <span className="text-gray-400 italic text-sm">Undetermined</span>
                    )}
                  </div>
                );
              })()}
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] z-50 md:left-64">
          <div className="max-w-5xl mx-auto flex justify-between items-center">
            {step === 1 ? (
              <>
                <div className="flex items-center gap-3">
                  <div className="bg-indigo-100 p-2.5 rounded-lg text-indigo-700"><Users size={24} /></div>
                  <div>
                    <div className="text-sm font-bold text-gray-500">Selected for Knockouts</div>
                    <div className="text-xl font-black text-gray-900">{selectedIds.length} Players</div>
                  </div>
                </div>
                <button 
                  onClick={() => {
              ...
              <>
                <button onClick={() => setStep(1)} className="text-gray-500 hover:text-gray-800 font-bold px-4 py-2 rounded-lg flex items-center gap-2">
