  ...
  const handlePhotoUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = async () => {
        ...
        const MAX_SIZE = 250; // Keep it very small to fit in the database
        let width = img.width;
        ...
        // Convert the shrunk canvas back into a compressed JPEG text string
        const base64String = canvas.toDataURL("image/jpeg", 0.6);

        try {
          // Save the text string directly to the existing player document
          await setDoc(doc(db, "players", playerData.id), {
      ...
      <div className="max-w-md mx-auto min-h-screen bg-gray-50 pb-24 relative">
        ...
        <div className="p-4">
          <div className="bg-blue-50 text-blue-800 p-4 rounded-lg border border-blue-100 mb-6 text-sm">
            Please check the boxes for <strong>all</strong> time slots when you are available to play.
          </div>
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <div className="mt-1 inline-block bg-white/20 px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wide">
                Category {playerData.category || "B"}
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <button onClick={openAvailability} className="w-full p-4 flex justify-between border-b border-gray-100 hover:bg-gray-50 text-left items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-bold text-gray-800">My Availability</div>
                <div className="text-xs text-gray-500">{playerData.availability?.length > 0 ? `${playerData.availability.length} slots saved` : "Required: Select slots"}</div>
              </div>
            </div>
